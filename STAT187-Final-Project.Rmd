...
## TO DO: 
- rearrange the levels of the factor so the relationship duration groups are in order: <1, 1 - 5, etc...
- place graphs below each other
- arrange how often couples sleep in separate beds in order of frequency as well: Never, once a year, once a month, few times per week, every night
...
plot_grid(rel_length_bar, prop_age_bar, nrows = 1, ncols = 1) 
...
#TO DO
#the two dots shouldn't be by themselves
# (Can't start sleeping in separate beds after 20 years if they've only been in the relationship for 1 - 5 years)
# try jittering the points less so it is more apparent to the reader which groups the dot belongs to. (default jitter choices may cause the points to move a little too much)
...
Can we predict how often a couple will be likely to sleep in separate beds based on Relationship Status, Relationship Length, Age, and Current Occupation?

The feature we want to classify is How_Often_Sep_Beds (how often couples sleep separately). We will be using a decision tree to model the data because each of the features which we are analyzing are categorical variables and the target variable is also categorical. 
...
sleeping_sep_ML <- sleeping_alone_data %>% 
  ...
  mutate(Frequency_Sleeping_Sep = factor(How_Often_Sep_Beds, 
                            levels = c("Never", "A few times per month", "A few times per week", "Once a year or less", "Once a month or less", "Every night"),
                            labels = c("Never", "Sometimes", "Sometimes", "Sometimes", "Sometimes", "Always"))) %>%
  dplyr::select(-How_Often_Sep_Beds)  
  


# Reads an R script with a function that divides the data into training and testing data
#source("forming training and testing sets function.R")

# Fixing the "random" splitting of the data
RNGversion('4.0.0')
set.seed(123)
# Using the function to create an object that stores the training and testing data sets
holdout_sleeping <- holdout_split(df = sleeping_sep_ML, pred = "Frequency_Sleeping_Sep", train_percent = 0.75)

names(holdout_sleeping)
...
train_df <- data.frame(holdout_sleeping$train_x,
                       How_Often_Sep_Beds = holdout_sleeping$train_y)
      
glimpse(train_df)
sleeping_tree <- rpart(formula = How_Often_Sep_Beds ~ .,
                     data = train_df,
                     method = "class",
